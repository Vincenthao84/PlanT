...
begin
  ...
  then
    ...
    if public.orders_taken_this_month(new.taken_by) >= 5 then
      raise exception 'Free tier limit reached: you can take up to 5 orders per month. Upgrade to continue.'
        using errcode = 'P0001';
    end if;
  end if;
